...
Sidebar.Geometry = function ( editor ) {
	...
	*/
    ...
    var convertRow = new UI.Row();
    convertRow.add( new UI.Text( 'Convert' ).setWidth( '50px' ) );
    convertRow.add( new UI.Button( 'TO GEOMETRY' ).setFontSize('11px').onClick( function () {
        ...
        if ( geometry instanceof THREE.BufferGeometry ) {
            ...
            if ( geometry.type == 'BoxBufferGeometry'){
                ...
                editor.execute( new SetGeometryCommand( object, new THREE.BoxGeometry().fromBufferGeometry( geometry ) ) );
            }
            
            if ( geometry.type == 'CylinderBufferGeometry'){

                editor.execute( new SetGeometryCommand( object, new THREE.CylinderGeometry().fromBufferGeometry( geometry ) ) );
            }
            
            if ( geometry.type == 'PlaneBufferGeometry'){

                editor.execute( new SetGeometryCommand( object, new THREE.PlaneGeometry().fromBufferGeometry( geometry ) ) );
            }
            
            if ( geometry.type == 'TorusBufferGeometry'){

                editor.execute( new SetGeometryCommand( object, new THREE.TorusGeometry().fromBufferGeometry( geometry ) ) );
            }
